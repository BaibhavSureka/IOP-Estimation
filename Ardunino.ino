...
const char* serverUrl = "http://192.168.125.175:5000/iop";
...
void takeSensorReading() {
    ...
    if (IOP > 50) {
        IOP = 25.0;  
        Serial.println("⚠️ High pressure detected, adjusting IOP for accuracy.");
    }

    // Store reading with filtering for noise
...
void loop() {
    ...
    if (readingCount < 10) {
        ...
        float finalIOP = (sortedIOP[3] + sortedIOP[4] + sortedIOP[5]) / 3.0;  
